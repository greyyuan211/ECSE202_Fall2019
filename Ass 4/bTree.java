...
/**
 * implement binary tree taught in class as myTree, and use myTree to execute stacking balls.
 * Based on A3,new functions added in A4: stop the movement of balls when stop button is clicked
 * 
 * @author Linwei Yuan
 *
 */
...
public class bTree extends GraphicsProgram{
	...
	//traverse_inorder method - recursively traverses tree in order (LEFT-Root-RIGHT) and prints each node.
		...
		public void changeStop(bNode root) {
			root.data.isStopped=!root.data.isStopped;
			if (root.left!=null) changeStop (root.left);
			if (root.right!=null) changeStop (root.right);
		}
	//root is only defined in bTree, so need to create another method here to call the recursion	
		public void stop () {
			changeStop(root);
		}
}
