...
/**
 * simulation the running balls
 * when the stop button is clicked, the while loop will stop the simulation
 * draw dots every 1s to trace the movement of the balls as long as the JToggleButton Trace is pressed (get status of button from bSim)
 * return the information stored in iBall to bTree
 * move the the balls to x,y to achieve stacking balls
 * @author Linwei Yuan
 *
 */
public class aBall extends Thread {
	...
	public volatile boolean isTracing ;
	private bSim link;
	...
	//two new constructors are added "link" and "isTracing", this ensures that tracing can be used in bSim class
	public aBall (double Xi, double Yi, double Vo, double theta, double bSize,
		Color bColor, double bLoss,bSim link,boolean isTracing) {
	//constructors
		this.Xi = Xi; 
		this.Yi = Yi;
		this.Vo = Vo;
		...
		GOval dot = new GOval((Xi-bSize)*SCALE,HEIGHT-2*bSize*SCALE,1,1);
		dot.setColor(bColor);
		dot.setFilled(true);
	...
	public void run () {
		...
		while(true) {
				while (isStopped); // block if the simulation is stopped
	    ...
	    // this if statement only happens when ball touches the ground 
	        ...
	        dot = new GOval((Xi-bSize)*SCALE,HEIGHT-2*bSize*SCALE,1,1);
	        dot.setLocation(ScrX+bSize*SCALE,ScrY+bSize*SCALE);
	        dot.setColor(bColor);
	        dot.setFilled(true);
	     //test is the trace button is pressed, if so, draw the dots	        
	         if (isTracing) {
				link.add(dot);
			}
