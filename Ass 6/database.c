...
/*  create addNode_ID function
 *  this part is translated from bTree in Java */
bNode *addNode_ID(bNode *root, SRecord *data){
    bNode *current; //add * to specify pointer in C
...
//  new data >= data at node, branch right
                if(current->right == NULL){             //leaf node
                    current->right = makeNode(data);    //attach new node here
                    break;
                }
                else{                                   //otherwise
    ...
    return root;//Note: don't need to return in Java
}

/*  create addNode_Name function
 *  this part is also translated from bTree in Java, there is no big difference from
 *  addNode_ID function*/
...
// If not empty, descend to the leaf node according to the input data
        ...
        current = root;
        while(true){
            if(strcmp(data->Last, current->Srec->Last)<0){ //==-1
...
//  new data > data at node, branch right
    ...
    return root; // don't need to return in Java
}
 ...
 * this part is also revised from bTree in Java with a non-recursive version
 * @param	int data   Data to be added
 * @return  bNode node Node created
 */
 bNode *makeNode(SRecord *data){
    bNode *node = (bNode*)malloc(sizeof(bNode));    //create new pointer
...
//  Revised from traverse_inorder method in Java
/**
 * inorder method - recursively traverses tree in order (LEFT-Root-RIGHT) and prints each node.
 */
void inorder(bNode *root){
...
//  search functions that will be implemented as binary search
void search_Name(bNode *root, char *data){
    bNode *current;
    current = root;
    while(true){
